let w = (c.width = window.innerWidth),
    ...
    hh = h / 2,
    opts = {
        strings: ["Feliz", "Aniversario!", "Natalia"],
        charSize: 30,
        charSpacing: 35,
        lineHeight: 40,
        cx: w / 2,
    ...
    TauQuarter = Tau / 4,
    letters = [],
    firstTime = true; // Flag para controlar a exibição do texto e da animação inicial
...
Letter.prototype.step = function () {
    if (this.phase === "firework") {
        if (firstTime && !this.spawned) {
            // Primeira exibição, apenas o texto e o fogo de artifício aparecem
            ++this.tick;
            ...
            // Segunda exibição - Centralizar o botão "entrar" na animação
            let button = document.getElementById("enterButton");
            button.style.position = "absolute";
            button.style.left = `${hw - button.offsetWidth / 2}px`;
            button.style.top = `${hh - button.offsetHeight / 2}px`;
        }

        // Continuação da animação do fogo de artifício
        ++this.tick;
